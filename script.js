...
// console.log(arr.splice(2)); // arr에서 2,3,4를 삭제함
arr.splice(-1); // 마지막 요소 삭제
console.log(arr);
arr.splice(1, 2); // 1부터 2개 삭제
console.log(arr);
...
console.log(arr2.reverse()); // 원본 배열에 영향을 미친다.
console.log(arr2);
...
console.log('------ FOREACH ------');
// forEach() => callback이 필요한 고차함수(higher-order function)
// forEach() => continue, break를 사용할 수 없다.
...
currenciesUnique.forEach(function (value, _, map) {
  console.log(`${value}: ${value}`); // set에는 key, index가 없음 =>
});
*/
...
// find => 새로운 배열을 만들지 않고 first element만 반환한다.
...
*/
/*
// Array from => mapping callback을 포함하고 있다.
...
1) Document.querySelectorAll => Nodelist 반환 (배열의 형태를 띠지만 배열은 아님)
2) Array.from 메서드를 이용해 배열로 반환
3) mapping 요소를 추가해 원하는 값으로 변환 (통화 표시 제거하고 숫자로 변경)


const movementsUI2 = [...document.querySelectorAll('.movements__value')];
// 역시 배열을 만들 수 있지만, Array.from을 사용하지 않았기 때문에 map을 따로 해주어야 함
...
// count ++ => 0을 반환 => ++count
// reduce method를 count하는 데에도 사용할 수 있다.
console.log(numDeposits1000);
